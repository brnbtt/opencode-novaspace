...
test("rejects traversal, symlinks, credentials and unreviewed machine paths before any upload", async () => {
  ...
  expect((await a.sync()).message).toContain("credentials")
  expect(host.writes()).toBe(0)
  await writeFile(join(a.paths.config, "opencode.json"), '{"plugins":["/Users/someone/local-plugin"]}')
  expect((await a.sync()).message).toContain("Machine-specific")
  expect(host.writes()).toBe(0)
  await a.configure({ repository: "example/profile", selected: ["settings"], allowMachinePaths: true })
  expect((await a.sync()).status).toBe("synced")
...
test("holds one cross-process lease and preserves existing files on first-connection conflicts", async () => {
  const host = server(), a = await machine(host.remote), b = await machine(host.remote)
  await configure(a); await configure(b)
  await writeFile(join(a.paths.config, "cli.json"), "remote")
  await writeFile(join(b.paths.config, "cli.json"), "local")
  await a.sync()
  expect((await b.sync()).status).toBe("conflict")
  expect(await readFile(join(b.paths.config, "cli.json"), "utf8")).toBe("local")
