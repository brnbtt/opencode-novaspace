    ...
    <box flexDirection="column">
      <For each={props.entries}>{(entry, index) => (
        <box flexDirection="row" justifyContent="space-between" minWidth={0} height={1}>
          <text flexGrow={1} minWidth={0} wrapMode="none" truncate fg={props.ctx.theme.text.subdued}>
            {`• ${entry.label}`}
          </text>
  ...
  return (
    ...
    >
      <box flexDirection="row" justifyContent="space-between" height={1}>
        <text fg={props.ctx.theme.text.default}><b>{props.title}</b></text>
        <text fg={props.ctx.theme.text.subdued}>{props.entries.length}</text>
      </box>
      <Show when={props.entries.length > 0} fallback={<text paddingLeft={1} fg={props.ctx.theme.text.subdued}>Nothing configured</text>}>
  ...
  return (
    ...
    >
      <box flexDirection="row" justifyContent="space-between" flexShrink={0} height={1}>
        <text fg={props.ctx.theme.text.default}><b>novaSpace</b></text>
        <text fg={props.ctx.theme.text.subdued}>{props.ctx.app?.version ? `v${props.ctx.app.version}` : ""}</text>
      </box>
      <text flexShrink={0} fg={props.ctx.theme.text.subdued}>OpenCode customization at a glance.</text>
      ...
      >
        <box flexDirection="column" gap={1} paddingRight={1}>
          <box id="setup-sync-panel" flexDirection="row" justifyContent="space-between" alignItems="center" paddingLeft={2} paddingRight={2} paddingTop={1} paddingBottom={1} backgroundColor={cardSurface(props.ctx.theme, 0.18)}>
            <text fg={props.ctx.theme.text.default}><b>Profile sync</b></text>
            <SetupActionLink id="setup-sync-open" ctx={props.ctx} label="Set up sync →" onPress={openSync} />
          </box>

          <Show when={!props.loading} fallback={<text fg={props.ctx.theme.text.subdued}>Inspecting setup…</text>}>
            <box flexDirection="column">
                ...
                return (
                  ...
                  >
                    <Show when={group.sections.length > 1}>
                      <text fg={props.ctx.theme.text.feedback.info.default}><b>{groupedSettings() ? "OpenCode settings" : "Shared configuration"}</b></text>
                    </Show>
                    <For each={group.sections}>{(section) => (
                      <box flexDirection="row" justifyContent="space-between" height={1} paddingLeft={group.sections.length > 1 ? 1 : 0}>
                        <text fg={props.ctx.theme.text.default}><b>{`${section.icon} ${section.label}`}</b></text>
                        <box flexDirection="row" gap={1}>
                          <text fg={props.ctx.theme.text.default}><b>{section.count}</b></text>
                          <text fg={section.healthy ? props.ctx.theme.text.feedback.success.default : props.ctx.theme.text.subdued}>{section.status}</text>
                        </box>
                      </box>
                    )}</For>
          ...
          <Show when={!settingsGrouped()}>
            <box flexDirection="column" paddingLeft={2} paddingRight={2} paddingTop={1} paddingBottom={1} backgroundColor={cardSurface(props.ctx.theme, 0.16)}>
              <text fg={props.ctx.theme.text.default}><b>OpenCode settings</b></text>
              <Show when={props.inventory.settings} fallback={<text fg={props.ctx.theme.text.subdued}>No settings file detected</text>}>
                {(target) => (
                  <box flexDirection="row" justifyContent="space-between" minWidth={0}>
                    <text flexGrow={1} minWidth={0} wrapMode="none" truncate fg={props.ctx.theme.text.subdued}>{displaySetupPath(props.ctx, target().path)}</text>
