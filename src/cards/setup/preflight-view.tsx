...
function PreflightGroup(props: {
  ...
  const color = () => props.color === "error"
    ...
    : props.color === "warning"
      ? props.ctx.theme.text.feedback.warning.default
      ...
      >
        ...
        <For each={props.items}>{(item) => (
          <box flexDirection="column" paddingLeft={1}>
            <text fg={props.ctx.theme.text.default}>{`• ${item.name}`}</text>
            <text fg={props.ctx.theme.text.subdued} wrapMode="word">{displaySetupPath(props.ctx, item.path)}</text>
            <Show when={item.detail}><text fg={props.ctx.theme.text.subdued} wrapMode="word">{item.detail}</text></Show>
          </box>
        )}</For>
      </box>
    </Show>
  ...
  return (
    <box flexDirection="column" gap={1}>
      <box flexDirection="row" justifyContent="space-between" height={1}>
        <text fg={props.ctx.theme.text.default}><b>Standardization preflight</b></text>
        <SetupActionLink id="setup-sync-back" ctx={props.ctx} label="← Back" onPress={props.back} />
      </box>
      <Show when={!props.loading} fallback={<text fg={props.ctx.theme.text.subdued}>Inspecting global and project setup…</text>}>
        <Show when={props.preflight} fallback={<text fg={props.ctx.theme.text.feedback.error.default}>{props.error ?? "Could not inspect setup"}</text>}>
          {(preflight) => (
            <>
              <box flexDirection="column" paddingLeft={2} paddingRight={2} paddingTop={1} paddingBottom={1} backgroundColor={cardSurface(props.ctx.theme, 0.18)}>
                <box flexDirection="row" justifyContent="space-between">
                  <text fg={props.ctx.theme.text.default}><b>Global profile</b></text>
                  <text fg={preflight().blockers.length ? props.ctx.theme.text.feedback.error.default : preflight().moves.length || preflight().warnings.length ? props.ctx.theme.text.feedback.warning.default : props.ctx.theme.text.feedback.success.default}>
                    <b>{preflightStatus(preflight())}</b>
                  </text>
                </box>
                <text fg={props.ctx.theme.text.subdued}>{displaySetupPath(props.ctx, preflight().root)}</text>
                <text fg={props.ctx.theme.text.subdued}>Read-only preview — no files have been moved.</text>
              </box>
              ...
              <Show when={preflight().moves.length > 0}>
                <box flexDirection="column" paddingLeft={2} paddingRight={2} paddingTop={1} paddingBottom={1} backgroundColor={cardSurface(props.ctx.theme, 0.14)}>
                  <text fg={props.ctx.theme.text.feedback.warning.default}><b>{`Proposed moves · ${preflight().moves.length}`}</b></text>
                  <For each={preflight().moves}>{(move) => (
                    <box flexDirection="column" paddingLeft={1}>
                      <text fg={props.ctx.theme.text.default}>{`• ${displaySetupPath(props.ctx, move.from)}`}</text>
                      <text fg={props.ctx.theme.text.subdued}>{`  → ${displaySetupPath(props.ctx, move.to)}`}</text>
                      <text fg={props.ctx.theme.text.subdued} wrapMode="word">{`${move.files} files · ${move.detail}`}</text>
                    </box>
                  )}</For>
                </box>
              </Show>
